...
def process_history_features(df, qualifier):
   values = []
   ...
   for item in df.to_numpy():
      ...
      if(not mp[item_id]):
         ...
         if( not last['visible'] or not qualifier(tags, osm_type) ): continue # If changed to something other than target in the last version or is deleted
...
def merge_history_geom(h, l, qualifier, item_type):
    ...
    merged = pd.merge(history, geom,  how='inner', left_on=['id','osm_type', 'ts'], right_on = ['id', 'osm_type', 'ts'])
...
def extract_indirect_indicators(qualifier, h, l):
